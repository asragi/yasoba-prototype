...
type SkillToSequenceFunc func(skill.SkillId) EventSequenceId

func ToEventSequenceId(skillId skill.SkillId) EventSequenceId {
...
func CreateServeBattleEventSequence() ServeBattleEventSequenceFunc {
	...
	register := func(id skill.SkillId, rows []BattleEventRow) {
	...
	register(skill.SkillIdNormalTackle, normalAttack)
				...
				for _, row := range sequence.Rows {
					...
					case *DisplayDamageEvent:
						...
						for target, damages := range damageMap {
							allDamage := func() skill.Damage {
								var result skill.Damage = 0
