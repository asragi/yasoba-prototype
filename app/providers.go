...
type battleSceneResult struct {
	BattleScene *scene.BattleScene
	NewText     widget.NewTextFunc
}

func provideDrawing() *frontend.Drawing {
	return frontend.NewDrawing()
}

func provideDebug(result *battleSceneResult) *debug.Debug {
	return debug.CreateDrawParameters(result.NewText)
}

func provideBattleSceneResult(cfg Config) (*battleSceneResult, error) {
	...
	textServer, err := text.LoadFromYAML(cfg.TextDataPath)
	if err != nil {
		return nil, err
	}
	...
	newWindow := widget.CreateNewWindow(resource, cfg.GameWidth, cfg.GameHeight)
